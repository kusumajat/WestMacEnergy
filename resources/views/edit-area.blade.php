        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">Edit Area</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form method="POST" action="{{ route('areas.update', $id) }}" enctype="multipart/form-data">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="hidden" id="color_area" name="color" value="">
...
@section('scripts')
    ...
    <script>
        ...
        function initColorSelection() {
            ...
            const colorInput = document.querySelector('#color_area');
            const previewColor = document.querySelector('#preview-color-area');
            const colorName = document.querySelector('#color-name-area');
        ...
        map.on('draw:edited', function(e) {
            ...
            layers.eachLayer(function(layer) {
                ...
                $('#preview-image-area').attr('src', "{{ asset('storage/images') }}/" + properties
                    .image);

                // Set color if available
                if (properties.color) {
                    $('#color_area').val(properties.color);
                    $('#preview-color-area').css('background-color', properties.color);
                    $('#color-name-area').text(colorNames[properties.color] || 'Custom');
    ...
    <script>
        ...
        var area = L.geoJson(null, {
            ...
            onEachFeature: function(feature, layer) {
                // memasukkan layer area ke dalam drawnItems
                drawnItems.addLayer(layer);
                ...
                layer.on({
                    click: function(e) {
                        ...
                        $('#geom_area').val(objectGeometry);
                        $('#preview-image-area').attr('src', "{{ asset('storage/images') }}/" +
                            feature.properties.image);

                        // Set color if available
                        if (feature.properties.color) {
                            $('#color_area').val(feature.properties.color);
                            $('#preview-color-area').css('background-color', feature.properties
                                .color);
                            $('#color-name-area').text(colorNames[feature.properties.color] ||
                                'Custom');
                            ...
                            $('#color_area').val('#FF0000');
                            $('#preview-color-area').css('background-color', '#FF0000');
                            $('#color-name-area').text('Red');
                        }
