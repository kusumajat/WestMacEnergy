...
@section('content')
    <div class="container py-5">
        <div class="page-header">
            <h1>Data Explorer</h1>
            <p>Find and view all geographic data. Use the search bar below to look for specific points, lines, or areas.</p>
        </div>
                        ...
                        @forelse ($points as $index => $p)
                            <tr>
                                ...
                                <td>
                                    @if ($p->image)
                                        <img src="{{ asset('storage/images/' . $p->image) }}" alt="{{ $p->name }}"
                                            class="img-thumbnail-table" data-bs-toggle="tooltip"
                                            title="{{ $p->image }}">
                                    @else
                                ...
                                <td>
                                    @if ($p->created_at)
                                        <span
                                            class="badge rounded-pill bg-secondary bg-opacity-75">{{ $p->created_at->format('d M Y, H:i') }}</span>
                                    @else
                                        -
                                    @endif
                                </td>
                                <td>
                                    @if ($p->updated_at)
                                        <span class="badge rounded-pill"
                                            style="background-color: var(--warning-color); color: var(--dark-text);">{{ $p->updated_at->format('d M Y, H:i') }}</span>
                                    @else
                                        -
                                    @endif
                                </td>
            ...
            @if ($points->hasPages())
                <div class="pagination-container">
                    <div class="pagination-info">
                        Showing <strong>{{ $points->firstItem() }}</strong> to <strong>{{ $points->lastItem() }}</strong>
                        of <strong>{{ $points->total() }}</strong> results
                    </div>
                    <div>
                        {{ $points->links() }}
                    </div>
        ...
        <div class="card mb-5">
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($p->image)
                                        <img src="{{ asset('storage/images/' . $p->image) }}" alt="{{ $p->name }}"
                                            class="img-thumbnail-table" data-bs-toggle="tooltip"
                                            title="Click to view image: {{ $p->image }}">
                                    @else
                                        <span class="text-muted fst-italic">No Image</span>
                                    @endif
                                </td>
            ...
            @if ($polylines->hasPages())
                <div class="pagination-container">
                    <div class="pagination-info">
                        Showing <strong>{{ $polylines->firstItem() }}</strong> to
                        <strong>{{ $polylines->lastItem() }}</strong> of <strong>{{ $polylines->total() }}</strong>
                        results
                    </div>
                    <div>
                        {{ $polylines->links() }}
        ...
        <div class="card mb-4">
            ...
            @if ($polygons->hasPages())
                <div class="pagination-container">
                    <div class="pagination-info">
                        Showing <strong>{{ $polygons->firstItem() }}</strong> to
                        <strong>{{ $polygons->lastItem() }}</strong> of <strong>{{ $polygons->total() }}</strong> results
                    </div>

                    <nav aria-label="Page navigation">
                        <ul class="pagination mb-0">
                            <li class="page-item {{ $polygons->onFirstPage() ? 'disabled' : '' }}">
                                <a class="page-link" href="{{ $polygons->previousPageUrl() }}" aria-label="Previous">
                                    <span aria-hidden="true">&laquo;</span>
                                </a>
                            </li>

                            @foreach ($polygons->getUrlRange(1, $polygons->lastPage()) as $page => $url)
                                @if ($page == $polygons->currentPage())
                            ...
                            <li class="page-item {{ !$polygons->hasMorePages() ? 'disabled' : '' }}">
                                <a class="page-link" href="{{ $polygons->nextPageUrl() }}" aria-label="Next">
